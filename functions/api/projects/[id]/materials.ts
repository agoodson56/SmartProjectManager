...
export const onRequestPost: PagesFunction<Env> = async (context) => {
    try {
        ...
        const { name, quantity, labor_hours_per_unit, unit_cost, is_addon } = await context.request.json() as any;
        ...
        ).bind(projectId, name, quantity, labor_hours_per_unit, unit_cost || 0, is_addon ? 1 : 0).run();
